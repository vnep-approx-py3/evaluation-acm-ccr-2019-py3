...
python -m vnep_approx.cli start-experiment sample_randround_execution.yml 0 10000 --concurrent 4 --overwrite_existing_intermediate_solutions --remove_intermediate_solutions
move_logs_and_output log_cactus_execution

#run randomized rounding algorithm using separation lp with dynvmp
python -m vnep_approx.cli start-experiment sample_rr_seplp_optdynvmp.yml 0 10000 --concurrent 4 --overwrite_existing_intermediate_solutions --remove_temporary_scenarios --remove_intermediate_solutions
move_logs_and_output log_seplp_dynvmp_execution

#extract data to be plotted
python -m evaluation_ifip_networking_2018.cli reduce-to-plotdata-randround-pickle sample_scenarios_results_cactus.pickle
